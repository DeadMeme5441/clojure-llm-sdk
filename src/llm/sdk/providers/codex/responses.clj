...
(defn build-request-codex
  ...
  (let [model (:request/model request)
        ...
        explicit-instructions (when (and (seq messages) (= (:message/role (first messages)) :system))
                                (t/content->string (:message/content (first messages))))
        ;; Codex backend requires instructions; fall back to a default
        instructions (or explicit-instructions (when backend? "You are a helpful assistant."))
        payload-messages (if explicit-instructions (rest messages) messages)
